...
export function ChartAreaInteractive() {
  ...
  // null = no explicit choice yet; mobile defaults to the shorter range.
  const [selectedRange, setSelectedRange] = React.useState<string | null>(null)
  const timeRange = selectedRange ?? (isMobile ? "7d" : "90d")
  const rangeLabel = RANGE_LABELS[timeRange] ?? RANGE_LABELS["90d"]
      ...
      <CardHeader>
        ...
        <CardAction>
          <ToggleGroup
            ...
            value={[timeRange]}
            onValueChange={(value) => {
              // Ignore toggle-off: one range is always selected.
              if (value[0]) {
                setSelectedRange(value[0])
              }
            }}
            variant="outline"
